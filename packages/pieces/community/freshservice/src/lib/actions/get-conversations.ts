...
export const getTicketConversations = createAction({
  auth: freshserviceAuth,
  name: 'get_ticket_conversation',
  displayName: 'Get Ticket Conversation',
  description: 'List ticket conversations',
  props: {
  ...
  async run(ctx) {
    ...
    const res = await freshserviceRequest(auth, {
      ...
      path: `/tickets/${ctx.propsValue.ticket_id}/conversations`,
    });
    return res.body;
  },
});
